...
module.exports = React.createClass({displayName: 'exports',
  ...
  setupPlayer: function() {
    ...
    this.scplayer.on('scplayer.track.resumed', function() {
      ...
      if (Global.currentIdx === 0 && Global.songs.length > 1) {
        ...
        self.setForwardButtonEnabled(true);
  ...
  setForwardButtonEnabled: function(bool) {
    if (bool) {
      $(this.refs.forwardButton.getDOMNode()).removeClass('disabled').addClass('enabled');
    } else {
      $(this.refs.forwardButton.getDOMNode()).removeClass('enabled').addClass('disabled');
    }
  },

  setPlayButtonEnabled: function(bool) {
    if (bool) {
      $(this.refs.playPauseButton.getDOMNode()).removeClass('disabled').addClass('enabled');
    } else {
      $(this.refs.playPauseButton.getDOMNode()).removeClass('enabled').addClass('disabled');
    }
  },
  ...
  onBack: function() {
    if (Global.currentIdx > 0) {
      this.setState({currentIdx: Global.currentIdx - 1, currentSong: Global.songs[Global.currentIdx - 1]});
      this.scplayer.prev();
    }
  },
