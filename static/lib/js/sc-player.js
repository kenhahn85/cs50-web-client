...
//v0.9.6
window.SoundCloudPlayer = function(tracks, config){
  var defaults = {
      loop: false
      , start_on: 0
      , autoplay: false
  ...
  this.next = function(autoplay){
    ...
    _this.log(_this.tracks)
    _this.log(_this.current_track_index);
...
//    url = url.replace(/https?\:\/\/soundcloud\.com/gi, "");
    ...
    var streamIdx = url.indexOf("/stream");
    url = url.substring(0, streamIdx) + ".json";
    ...
    jQuery.ajax({
      ...
      , error: function(jqXHR, textStatus, errorThrown){
        PubSub.publishSync("SOUNDCLOUD.404");
